 ...
 * @brief ROS 1 core node for iTFS LiDAR
...
geometry_msgs::TransformStamped make_transform(
    const ros::Time &stamp, const std::string &parent, const std::string &child,
...
class ItfsCoreNode {
  public:
    ItfsCoreNode() : private_nh_("~") {
        ...
        while (ros::ok() && !lidar_->Ready()) std::this_thread::sleep_for(100ms);
        if (!ros::ok()) {
        ...
        ROS_INFO("iTFS::LiDAR is ready. Waiting for up to %d devices.",
                    iTFS::max_device);
    }
  ...
  private:
    ...
    void declare_parameters() {
        ...
        default_mapping_file_ = ros::package::getPath("ilidar_itfs_ros") + "/dat/iTFS-110.dat";
        ...
        defaults_.sensor_qos_depth = static_cast<int>(std::max<int>(
            immutable_parameter<int>("sensor_qos_depth", 5), 1));
        diagnostics_qos_depth_ = static_cast<int>(std::max<int>(
            immutable_parameter<int>("diagnostics_qos_depth", 10), 1));
    ...
    DeviceSettings load_device_settings(DeviceContext &context) {
        ...
        // ROS 1 private parameters use slash-separated namespaces.
        ...
        settings.sensor_qos_depth = static_cast<int>(std::max<int>(device_parameter<int>(
...
#undef LOAD_POSE
    ...
    DeviceContext *initialize_context(iTFS::device_t *device, const NormalizedInfo &info) {
        ...
        for (const auto &other : contexts_) {
            if (other && other->sensor_sn == info.sensor_sn) {
                ROS_ERROR( "Duplicate sensor SN#%u.", info.sensor_sn);
        ...
        try {
            ...
            if (context->settings.publish_pointcloud) {
                try {
                    ...
                    ROS_ERROR("%s PointCloud2 will remain unavailable.", error.what());
                }
            }
            ...
            ROS_INFO( "D#%d SN#%u initialized at %s.", device->idx,
    ...
    void on_data(iTFS::device_t *device) {
        ...
        if (device->data.mode != context->info.capture_mode ||
            ...
            context->faulted.store(true);
            ROS_ERROR("D#%d capture mode/row changed while running.", device->idx);
            return;
        }
    ...
    void prepare_image(sensor_msgs::Image &message, int height,
    ...
    void copy_image(const uint16_t *source, sensor_msgs::Image &message) const {
    ...
    void publish_pointcloud(DeviceContext &context, const ItfsFrameSnapshot &frame,
        ...
        // ROS 1 flattens a newly resized cloud; restore the sensor's organized layout.
        cloud.height = height;
        cloud.width = iTFS::max_col;
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "ilidar_core");
    try {
        ItfsCoreNode node;
        ros::spin();
    } catch (const std::exception &error) {
