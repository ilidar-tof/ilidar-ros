...
struct DeviceContext {
    ...
    sensor_msgs::Image amplitude_msg;
    sensor_msgs::Image intensity_msg;
    sensor_msgs::Image confidence_msg;
...
template <typename T>
diagnostic_msgs::KeyValue key_value(const std::string &key, const T &value) {
...
geometry_msgs::TransformStamped make_transform(const ros::Time &stamp,
...
class LiteCoreNode {
  public:
    ...
    LiteCoreNode() : private_nh_("~") {
        ...
        diagnostics_pub_ = nh_.advertise<diagnostic_msgs::DiagnosticArray>(
            "/diagnostics", diagnostics_qos_depth_);
        ...
        diagnostics_timer_ = nh_.createTimer(
            ros::Duration(std::chrono::duration<double>(diagnostics_publish_period).count()),
            [this](const ros::TimerEvent &) { publish_diagnostics(); });
        subscription_timer_ = nh_.createTimer(
            ros::Duration(std::chrono::duration<double>(subscription_poll_period).count()),
            [this](const ros::TimerEvent &) { update_subscription_masks(); });

        ROS_INFO("iTFS::LITE is ready. Waiting for up to %d devices.", iTFS::max_device);
    }
  ...
  private:
    // ROS 1 parameters are loaded once and cached. Parameter-server changes are
    // intentionally ignored until restart, matching the ROS 2 immutable policy.
    template <typename T>
    T immutable_parameter(const std::string &name, const T &default_value) {
        T value;
        private_nh_.param<T>(name, value, default_value);
        return value;
    }
    ...
    void declare_parameters() {
        ...
        default_settings_.parent_frame_id =
            immutable_parameter<std::string>("parent_frame_id", "base_link");
        default_settings_.publish_tf = immutable_parameter<bool>("publish_tf", true);
        default_settings_.publish_depth = immutable_parameter<bool>("publish_depth", true);
        default_settings_.publish_amplitude = immutable_parameter<bool>("publish_amplitude", true);
        default_settings_.publish_intensity = immutable_parameter<bool>("publish_intensity", true);
        default_settings_.publish_confidence = immutable_parameter<bool>("publish_confidence", true);
        default_settings_.publish_camera_info = immutable_parameter<bool>("publish_camera_info", true);
        default_settings_.publish_pointcloud = immutable_parameter<bool>("publish_pointcloud", true);
        default_settings_.publish_pointcloud_amplitude =
            immutable_parameter<bool>("publish_pointcloud_amplitude", true);
        default_settings_.publish_info = immutable_parameter<bool>("publish_info", true);
        default_settings_.publish_diagnostics = immutable_parameter<bool>("publish_diagnostics", true);
        default_settings_.sensor_qos_depth =
            std::max(immutable_parameter<int>("sensor_qos_depth", 5), 1);
        diagnostics_qos_depth_ =
            std::max(immutable_parameter<int>("diagnostics_qos_depth", 10), 1);
        ...
        default_settings_.mount_translation_x = immutable_parameter<double>("mount_translation_x", 0.0);
        default_settings_.mount_translation_y = immutable_parameter<double>("mount_translation_y", 0.0);
        default_settings_.mount_translation_z = immutable_parameter<double>("mount_translation_z", 0.0);
        default_settings_.mount_rotation_roll = immutable_parameter<double>("mount_rotation_roll", 0.0);
        default_settings_.mount_rotation_pitch = immutable_parameter<double>("mount_rotation_pitch", 0.0);
        default_settings_.mount_rotation_yaw = immutable_parameter<double>("mount_rotation_yaw", 0.0);
        default_settings_.optical_translation_x = immutable_parameter<double>("optical_translation_x", 0.0);
        default_settings_.optical_translation_y = immutable_parameter<double>("optical_translation_y", 0.0);
        default_settings_.optical_translation_z = immutable_parameter<double>("optical_translation_z", 0.0);
        default_settings_.optical_rotation_roll = immutable_parameter<double>("optical_rotation_roll", -half_pi);
        default_settings_.optical_rotation_pitch = immutable_parameter<double>("optical_rotation_pitch", 0.0);
        default_settings_.optical_rotation_yaw = immutable_parameter<double>("optical_rotation_yaw", -half_pi);
    ...
    void create_publishers(DeviceContext &context) {
        if (context.settings.publish_depth) {
            context.depth_pub = nh_.advertise<sensor_msgs::Image>(
                topic_name(context.topic_prefix, "/depth/image_raw"), context.settings.sensor_qos_depth);
        }
        if (context.settings.publish_camera_info) {
            context.depth_camera_info_pub = nh_.advertise<sensor_msgs::CameraInfo>(
                topic_name(context.topic_prefix, "/depth/camera_info"), context.settings.sensor_qos_depth);
        }
        if (context.settings.publish_amplitude) {
            context.amplitude_pub = nh_.advertise<sensor_msgs::Image>(
                topic_name(context.topic_prefix, "/amplitude/image_raw"), context.settings.sensor_qos_depth);
        }
        if (context.settings.publish_intensity) {
            context.intensity_pub = nh_.advertise<sensor_msgs::Image>(
                topic_name(context.topic_prefix, "/intensity/image_raw"), context.settings.sensor_qos_depth);
        }
        if (context.settings.publish_confidence) {
            context.confidence_pub = nh_.advertise<sensor_msgs::Image>(
                topic_name(context.topic_prefix, "/confidence/image_raw"), context.settings.sensor_qos_depth);
        }
        if (context.settings.publish_pointcloud) {
            context.points_pub = nh_.advertise<sensor_msgs::PointCloud2>(
                topic_name(context.topic_prefix, "/points"), context.settings.sensor_qos_depth);
        }
        if (context.settings.publish_info) {
            context.info_pub = nh_.advertise<std_msgs::String>(
                topic_name(context.topic_prefix, "/info"), 1, true);
        }
    }
    ...
    // Overlay devices/ilidar_lite_<SN>/ values on the common startup defaults.
    // The resulting settings object is never changed while the worker runs.
    DeviceSettings load_device_settings(DeviceContext &context) {
        ...
        settings.sensor_qos_depth = std::max(
            device_parameter<int>(context, "sensor_qos_depth",
                                  default_settings_.sensor_qos_depth), 1);
    ...
    DeviceContext *initialize_context(iTFS::lite_device_t *device) {
        ...
        for (const auto &other : contexts_) {
            if (other && other->sensor_sn == device->info_v3.sensor_sn) {
                ROS_ERROR( "Duplicate sensor SN#%u on D#%d and D#%d.",
        ...
        try {
            ...
            ROS_INFO( "D#%d SN#%u initialized at %s.",
                        device->idx, context->sensor_sn, context->topic_prefix.c_str());
            return context;
        } catch (const std::exception &exc) {
            context_init_failed_[device->idx].store(true, std::memory_order_relaxed);
            ROS_ERROR( "D#%d SN#%u initialization failed: %s",
    ...
    // ROS graph queries stay on the ROS spinner thread. The SDK callback
    // consumes only this cached byte, avoiding middleware locks in reception.
    ...
    void handle_lidar_data(iTFS::lite_device_t *device) {
        ...
        context->pending_frame->stamp = ros::Time::now();
        ...
        if ((request.camera_info || request.pointcloud) &&
            ...
            ROS_INFO( "D#%d intrinsic and reconstruction map are ready.", device->idx);
        }
    ...
    // string formatting, logging, and ROS publication out of the SDK thread.
    void publish_diagnostics() {
        ...
        for (int device_idx = 0; device_idx < iTFS::max_device; ++device_idx) {
            ...
            status.level = context_faulted ? diagnostic_msgs::DiagnosticStatus::ERROR :
            ...
            status.values = {
                ...
                key_value("ros_stamp_nanosec", array.header.stamp.nsec),
    ...
    void handle_info_packet(iTFS::lite_device_t *device) {
        ...
        if (device->info_v3.sensor_sn != context->sensor_sn) {
            ROS_ERROR( "D#%d changed SN from %u to %u.", device->idx,
    ...
    void update_output_mode_log(DeviceContext &context) {
        ...
        if (repeat_due) {
            ROS_WARN( "D#%d SN#%u non-native ROS image modes: %s",
    ...
    bool has_subscribers(const ros::Publisher &publisher) const {
    ...
    void prepare_image_message(sensor_msgs::Image &msg,
        ...
        msg.step = static_cast<sensor_msgs::Image::_step_type>(
    ...
    void publish_image(const ros::Publisher &publisher,
    ...
    void publish_camera_info(DeviceContext &context, const LiteFrameSnapshot &frame) {
        sensor_msgs::CameraInfo &msg = context.camera_info_msg;
        if (!context.camera_info_initialized) {
            ...
            if (std::abs(k4) > std::numeric_limits<double>::epsilon()) {
                ROS_WARN( "SN#%u CameraInfo omits unsupported SDK k4 (r^8).",
                            context.sensor_sn);
            }
    ...
    void publish_pointcloud(DeviceContext &context,
        ...
        sensor_msgs::PointCloud2 &cloud = context.points_msg;
        ...
        if (!context.points_schema_initialized) {
            if (include_amplitude) {
                modifier.setPointCloud2Fields(4,
                                              ...
                                              "amplitude", 1, sensor_msgs::PointField::FLOAT32);
            } else {
        ...
        // ROS 1 PointCloud2Modifier::resize() flattens an empty cloud to 1xN.
        // Restore the sensor's organized layout after allocating the storage.
        cloud.height = iTFS::lite_max_row;
        cloud.width = iTFS::lite_max_col;
        ...
        std::unique_ptr<sensor_msgs::PointCloud2Iterator<float>> iter_amplitude;
        if (include_amplitude) {
            iter_amplitude.reset(
                new sensor_msgs::PointCloud2Iterator<float>(cloud, "amplitude"));
        }
    ...
    ros::Timer subscription_timer_;
    std::unique_ptr<tf2_ros::StaticTransformBroadcaster> static_tf_broadcaster_;
};

int main(int argc, char **argv) {
    ros::init(argc, argv, "ilidar_lite_core");
    try {
        // ros::spin owns timer callbacks; SDK callbacks and per-SN workers run
        // on their dedicated threads and synchronize through DeviceContext.
        LiteCoreNode node;
        ros::spin();
    } catch (const std::exception &exc) {
